...
const PersonCard = ({ image, name, role, affiliation, bio, linkedin, website, isExpanded, onToggle }: PersonCardProps) => {
  return (
    <div 
      className="group self-start card-glow overflow-hidden cursor-pointer"
      onClick={onToggle}
    >
      <div className="p-4">
        <div className="w-full aspect-square rounded-xl overflow-hidden bg-muted mb-4">
          <img 
            src={image} 
            alt={name} 
            className="w-full h-full object-cover saturate-75 group-hover:saturate-100 transition-all duration-500 group-hover:scale-105" 
          />
        </div>
...
export const TeamSection = () => {
  ...
  return (
    <section id="team" className="pt-32 pb-16 lg:pt-40 section-elegant">
      <div className="section-container">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-16">
          {teamMembers.map((member) => (
            <PersonCard 
              key={member.name} 
              {...member} 
              isExpanded={expandedCards.has(member.name)}
        ...
        <div className="text-center mb-10">
          <h3 className="text-2xl font-semibold text-foreground">Advisory Board</h3>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {advisors.map((advisor) => (
            <PersonCard 
              key={advisor.name} 
              {...advisor} 
              isExpanded={expandedCards.has(advisor.name)}
              onToggle={() => handleToggle(advisor.name)}
            />
          ))}
        </div>
      </div>
